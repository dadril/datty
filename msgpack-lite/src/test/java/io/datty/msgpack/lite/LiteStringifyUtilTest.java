package io.datty.msgpack.lite;

import org.junit.Assert;
import org.junit.Test;

import io.datty.msgpack.lite.util.LiteStringifyUtil;
import io.datty.msgpack.lite.util.LiteStringifyUtil.NumberType;


/**
 * LiteStringifyUtilTest
 * 
 * @author Alex Shvid
 *
 */

public class LiteStringifyUtilTest {

	@Test
	public void testNull() {
		
		Assert.assertEquals(NumberType.NAN, LiteStringifyUtil.detectNumber(null));
		
	}
	
	@Test
	public void testEmpty() {
		
		Assert.assertEquals(NumberType.NAN, LiteStringifyUtil.detectNumber(""));
		
	}
	
	@Test
	public void testLong() {
		
		Assert.assertEquals(NumberType.LONG, LiteStringifyUtil.detectNumber("123456789"));
		
	}
	
	@Test
	public void testMinusLong() {
		
		Assert.assertEquals(NumberType.LONG, LiteStringifyUtil.detectNumber("-123456789"));
		
	}
	
	@Test
	public void testInvalidMinusLong() {
		
		Assert.assertEquals(NumberType.NAN, LiteStringifyUtil.detectNumber("123-456789"));
		
	}
	
	@Test
	public void testDouble() {
		
		Assert.assertEquals(NumberType.DOUBLE, LiteStringifyUtil.detectNumber("123456789.0"));
		
	}
	
	@Test
	public void testDoubleE() {
		
		Assert.assertEquals(NumberType.DOUBLE, LiteStringifyUtil.detectNumber("1.23456789E8"));
		
	}
	
	@Test
	public void testInvalidDoubleE() {
		
		Assert.assertEquals(NumberType.NAN, LiteStringifyUtil.detectNumber("1.2345E789E8"));
		
	}
	
	@Test
	public void testInvalidDouble() {
		
		Assert.assertEquals(NumberType.NAN, LiteStringifyUtil.detectNumber("12345.6789.0"));
		
	}
	
	@Test
	public void testMinusDouble() {
		
		Assert.assertEquals(NumberType.DOUBLE, LiteStringifyUtil.detectNumber("-123456789.0"));
		
	}
	
	@Test
	public void testMinusDoubleE() {
		
		Assert.assertEquals(NumberType.DOUBLE, LiteStringifyUtil.detectNumber("-1.23456789E8"));
		
	}
	
	@Test
	public void testHexNull() {
		
		String hex = LiteStringifyUtil.toHex(null);
		
		Assert.assertNull(hex);
	}
	
	@Test
	public void testHexEmpty() {
		
		String hex = LiteStringifyUtil.toHex(new byte[] {});
		
		Assert.assertEquals("", hex);
		
	}
	
	@Test
	public void testHexSingle() {
		
		String hex = LiteStringifyUtil.toHex(new byte[] { 1 });
		
		Assert.assertEquals("01", hex);
		
	}
	
	@Test
	public void testHexAll() {
		
		byte[] all = new byte[256];
		for (int i = 0; i != 256; ++i) {
			all[i] = (byte) i;
		}
		
		String hex = LiteStringifyUtil.toHex(all);
		
		Assert.assertEquals("", hex);
		
	}
	
}
