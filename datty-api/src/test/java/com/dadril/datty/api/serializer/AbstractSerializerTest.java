...
public abstract class AbstractSerializerTest {
  ...
  protected void compareResult(ValueResult expected, ValueResult actual) {
    ...
    Assert.assertTrue(Arrays.equals(expected.get().getBackingValue(), actual.get().getBackingValue()));
    Assert.assertEquals(expected.get().getVersion(), actual.get().getVersion());
  ...
  protected void testSingle(DattySerializer serializer) throws IOException {
    ...
    GetOperation actual = (GetOperation) DattyIOUtil.parseOperation(blob, serializer);
    ...
    ValueResult actualRes = (ValueResult) DattyIOUtil.parseResult(blob, serializer);
  ...
  protected void testBatch(DattySerializer serializer) throws IOException {
    ...
    BatchOperation actual = (BatchOperation) DattyIOUtil.parseOperation(blob, serializer);
    ...
    BatchResult actualRes = (BatchResult) DattyIOUtil.parseResult(blob, serializer);
    
    Assert.assertEquals(batchRes.size(), actualRes.size());
    compareResult((ValueResult)batchRes.get(0), (ValueResult)actualRes.get(0));

    
  }
  
  protected long testSerializePerformace(DattySerializer serializer, int iterations) throws IOException {
  ...
  protected long testDeserializePerformace(DattySerializer serializer, int iterations) throws IOException {
